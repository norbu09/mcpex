defmodule Mcpex.Handlers.Initialization do
  ...
  def handle_exit(params, session_id) do
    Logger.info("Exit notification received for session #{session_id} with params: #{inspect(params)}")
    cleanup_session(session_id)
    :ok
  end
  ...
  defp store_session_info(session_id, client_info, client_capabilities, server_capabilities) do
    ...
    Logger.debug("Storing session info for #{session_id}: client_info=#{inspect(client_info)}, " <>
