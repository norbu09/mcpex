defmodule Mcpex.Application do
  ...
  def start(_type, _args) do
    ...
    default_rate_limiter_config = [
      # Default GenServer name
      name: RateLimiterServer,
      # Default ETS table name
      table_name: :mcpex_rate_limits_ets,
      # Default GC interval
      gc_interval: :timer.minutes(5),
      # Default minimal rule, ideally this comes from config files
      rules: [
        %{
          id: :default_mcp_request,
          limit: Application.get_env(:mcpex, :default_mcp_request_limit, 100),
          period:
            :timer.minutes(Application.get_env(:mcpex, :default_mcp_request_period_minutes, 1)),
    ...
    rate_limiter_opts =
      if Keyword.has_key?(rate_limiter_opts, :name) do
        rate_limiter_opts
      else
    ...
    Logger.info(
      "Starting Mcpex.RateLimiter.Server with effective options: #{inspect(rate_limiter_opts)}"
    )
