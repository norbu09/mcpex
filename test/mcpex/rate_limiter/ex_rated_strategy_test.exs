defmodule Mcpex.RateLimiter.ExRatedStrategyTest do
  # ETS tables might require async: false or careful naming
  ...
  describe "init/1" do
    test "initializes with given rules", %{strategy_state: strategy_state, rules: rules} do
      assert Map.get(strategy_state.rules, :test_default) ==
               Enum.find(rules, &(&1.id == :test_default))
    end
  end
  ...
  describe "check_and_update_limit/3" do
    test "allows requests within the limit for a rule", %{strategy_state: strategy_state} do
      ...
      {:ok, new_state1, details1} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "user1", :test_default)

      assert details1.remaining == 2
      assert details1.limit == 3

      {:ok, new_state2, details2} =
        ExRatedStrategy.check_and_update_limit(new_state1, "user1", :test_default)

      assert details2.remaining == 1

      {:ok, _new_state3, details3} =
        ExRatedStrategy.check_and_update_limit(new_state2, "user1", :test_default)
    ...
    test "denies requests exceeding the limit for a rule", %{strategy_state: strategy_state} do
      ...
      {:ok, state1, _} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "user2", :test_strict)

      {:error, :rate_limited, _state2, details} =
        ExRatedStrategy.check_and_update_limit(state1, "user2", :test_strict)
    ...
    test "handles different identifiers independently", %{strategy_state: strategy_state} do
      ...
      {:ok, state_userX, _} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "userX", :test_strict)

      {:error, :rate_limited, _, _} =
        ExRatedStrategy.check_and_update_limit(state_userX, "userX", :test_strict)

      # UserY also uses :test_strict, should be independent
      {:ok, _state_userY, detailsY} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "userY", :test_strict)

      # Limit is 1, so 1st request makes remaining 0
      assert detailsY.remaining == 0
      assert detailsY.limit == 1
    end

    test "handles different rules independently for the same identifier", %{
      strategy_state: strategy_state
    } do
      # Use :test_default (limit 3)
      {:ok, state1, _} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "user3", :test_default)

      {:ok, state2, _} = ExRatedStrategy.check_and_update_limit(state1, "user3", :test_default)

      # Use :test_strict (limit 1) for the same user3
      {:ok, state3, strict_details} =
        ExRatedStrategy.check_and_update_limit(state2, "user3", :test_strict)
      ...
      {:error, :rate_limited, _, _} =
        ExRatedStrategy.check_and_update_limit(state3, "user3", :test_strict)

      # But making another call to :test_default should still be okay (1 remaining)
      {:ok, _, default_details} =
        ExRatedStrategy.check_and_update_limit(state3, "user3", :test_default)

      # This is the 3rd call for :test_default
      assert default_details.remaining == 0
    end

    test "returns :ok with reason for unknown rule_name (fail open)", %{
      strategy_state: strategy_state
    } do
      {:ok, _new_state, details} =
        ExRatedStrategy.check_and_update_limit(strategy_state, "user4", :unknown_rule)
    ...
    test "waits for period to pass to allow requests again", %{strategy_state: strategy_state} do
      ...
      updated_rules =
        Map.put(strategy_state.rules, rule_id, %{
          id: rule_id,
          limit: 1,
          period: short_period_ms,
          strategy: ExRated.Strategy.FixedWindow
        })
      ...
      {:ok, state1, _} =
        ExRatedStrategy.check_and_update_limit(current_state, "user_wait", rule_id)

      # Second request, should be rate limited
      {:error, :rate_limited, state2, _} =
        ExRatedStrategy.check_and_update_limit(state1, "user_wait", rule_id)

      # Wait for longer than the period (e.g., period + 50ms)
      :timer.sleep(short_period_ms + 50)

      # Third request, should be allowed again
      {:ok, _, details_after_wait} =
        ExRatedStrategy.check_and_update_limit(state2, "user_wait", rule_id)
