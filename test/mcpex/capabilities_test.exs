defmodule Mcpex.CapabilitiesTest do
  ...
  describe "Tools capability" do
    ...
    test "executes calculator tool" do
      # Execute calculator
      result =
        Mcpex.Capabilities.Tools.handle_request(
          "tools/execute",
  ...
  describe "Prompts capability" do
    ...
    test "gets specific prompt" do
      # Get greeting prompt
      result =
        Mcpex.Capabilities.Prompts.handle_request(
          "prompts/get",
          %{"name" => "greeting"},
          "test-session"
        )
  ...
  describe "Resources capability" do
    test "lists resources from registry" do
      # Get resources
      resources =
        Mcpex.Capabilities.Resources.handle_request("resources/list", %{}, "test-session")
    ...
    test "reads resource content" do
      # Read text file
      result =
        Mcpex.Capabilities.Resources.handle_request(
          "resources/read",
      ...
      # Verify result
      assert {:ok, %{"contents" => [%{"uri" => "file://example.txt", "text" => content}]}} =
               result
